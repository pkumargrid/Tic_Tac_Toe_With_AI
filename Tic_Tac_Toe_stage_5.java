...
//    }
    ...
    public boolean user(boolean flag){
        try{
            ...
            c[0]--; c[1]--;
            if(board[c[0]][c[1]] != -1){
                throw new tictactoe.MyException("This cell is occupied! Choose another one!");
            }
            int val = cnt();
            if(val > 0){
                board[c[0]][c[1]] = 0;
    ...
    public int minimax(int comp,int symbol,int depth, boolean step){
        int condition = check();
        if(condition == 1){
            return 0;
        }
        else if(condition == 3 && comp == 0){
            return 1;
        }
        else if(condition == 3 && comp == 1){
            return -1;
        }
        else if(condition == 2 && comp == 0){
            return -1;
        }
        else if(condition == 2 && comp == 1){
            return 1;
        }
        if(step){
            ...
            for(int i = 0; i < 3; i++){
                for(int j = 0; j < 3; j++){
                    if(board[i][j] != -1) continue;
                    board[i][j] = symbol;
                    maxScore = Math.max(maxScore,minimax(comp,symbol^1,depth + 1,false));
                    board[i][j] = -1;
                }
            }
            return maxScore;
            ...
            for(int i = 0; i < 3; i++){
                for(int j = 0; j < 3; j++){
                    if(board[i][j] != -1) continue;
                    board[i][j] = symbol;
                    minScore = Math.min(minScore,minimax(comp,symbol^1,depth + 1,true));
                    board[i][j] = -1;
                }
            }
            return minScore;
    ...
    public int[] find(int symbol){
        ...
        for(int i =0; i < 3; i++){
            for(int j = 0; j < 3; j++){
                if(board[i][j] != -1) continue;
                board[i][j] = symbol;
                int val = minimax(symbol,symbol^1,0,false);
                if(bestScore < val){
                    bestScore = val;
                    c[0] = i;
                    c[1] = j;
    ...
    public void computer(int symbol,String level){
        ...
        if(level.equals("hard")){
            int[] corr = find(symbol);
            board[corr[0]][corr[1]] = symbol;
            return;
        }
    ...
    public boolean bad_param(String[] input){
        ...
        if(!input[1].equals("user") && !input[1].equals("medium")
                && !input[1].equals("easy") && !input[1].equals("hard")){
            return false;
        }
        return input[2].equals("user") || input[2].equals("medium") ||
                input[2].equals("easy") || input[2].equals("hard");
    }
    public void start(){
        init();
        initialize_board();
        while(true){
            try{
                ...
                if(!bad_param(input)){
                    throw new tictactoe.BadParamException("Bad parameters!");
                }
            ...
            catch(tictactoe.BadParamException e){
                System.out.println(e.getMessage());
            }
...
public class Tic_Tac_Toe_stage_5 {
    public static void main(String[] args) {
        // write your code here
        tictactoe.TicTacToe tt = new tictactoe.TicTacToe();
        tt.start();
    }
}
